...
class Node:
	...
	def process_node( self, all_words, my_words_idx, history, verbose ):
		...
		best_query = ""
		best_query_idx = 0
		best_entropy = len(all_words) * 100000 	# very high default entropy value
		if len( history ) == 0:
			best_query_idx = -1			# this ensures that the root node is split into children containing words of different lengths	
		else:
			for try_idx in my_words_idx:					# try every word in the dictionary as a query
				split_dict = {}
				cnt_entropy = 0
				for idx in my_words_idx:
					mask = self.reveal(all_words[ idx ], all_words[try_idx] )
				...
				for split in split_dict.values():
					cnt_entropy += len(split) * np.log2(len(split))
